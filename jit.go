...
// HAR YEAR KA ALAG DATABASE BANANE WALA FUNCTION
func initYearDB(year string) *sql.DB {
	...
	_, err = db.Exec(`
		...
		CREATE TABLE IF NOT EXISTS student_records (
			student_unique_id TEXT PRIMARY KEY, -- Single primary key ab chalega, kyunki file hi alag hai!
			academic_year TEXT,
			school_id TEXT,
			school_name TEXT,
			district_id TEXT,
...
func main() {
	...
	// Charo alag files initialize ho rahi hain
	for _, yr := range targetYearsList {
		dbConnections[yr] = initYearDB(yr)
		defer dbConnections[yr].Close()
	}
	logMessage("✅ All 4 Year Databases (Separate Files) initialized successfully.")
	...
	for rows.Next() {
		...
		// AAPKA ASLI SCRAPING LOGIC (BINA KISI GADBAD KE)
		for cid := startClass; cid <= endClass; cid++ {
			targetYears := []string{"2023-24"}
			if cid == startClass {
				// Sirf pehli class me baki ke saal judenge
	...
	if totalTasks == 0 {
		logMessage("❌ OOPS! Task Queue khali hai.")
		return
	}
	...
	for records := range resultChan {
		...
		if len(buffer) >= BatchSize || processedCount == totalTasks {
			if len(buffer) > 0 {
				// DATA KO SAHI SAAL KE DB FILE ME ROUTE KARNE WALA FUNCTION CALL
			...
			logMessage("⚡ Progress: %d/%d Done | Total Actual Saved Across DBs: %d | Speed: %.2f req/sec",
				processedCount, totalTasks, totalSaved, speed)
		}
	}

	logMessage("🎉 Done! All students divided into their respective Year DB files safely.")
}
	...
	for task := range tasks {
		...
		for _, std := range apiStudents {
			...
			record := StudentRecord{
				...
				AcademicYear:    task.AcademicYear, // Task ka year direct pass kiya taaki data mix na ho
...
// BATCH DATA KO SAHI DB FILE ME INTEGRATE KARNE KA REAL LOGIC
func saveBatchByYear(connections map[string]*sql.DB, records []StudentRecord) int {
	// Records ko unke academic_year ke hisab se map me group karein
	...
	// Har year group ke liye uske sahi database connection par transaction chalayein
