...
func main() {
	...
	fmt.Println("2 -> Test Mode / Custom UDISE Input (Fetches Actual Classes from DB)")
	...
	if modeInput == "2" {
		...
		for i := 1; i <= schoolCount; i++ {
			...
			err := schoolDB.QueryRow("SELECT class_frm, class_to FROM schools WHERE school_code = ?", udise).Scan(&classFrm, &classTo)
			...
			if err == nil {
				...
				logMessage("🎯 School %s found in DB! Classes: %d to %d", udise, startClass, endClass)
			} else {
				logMessage("⚠️ School %s not found in DB! Defaulting to classes 1 to 12.", udise)
			}
		...
		// --- STANDARD BULK MODE (OPTIMIZED FOR TASK COUNT) ---
		logMessage("📦 Scanning School Database for Bulk tasks...")
		...
		for rows.Next() {
			...
			// Task calculation strictly aligned with Dynamic Logic
			for cid := startClass; cid <= endClass; cid++ {
				var targetYears []string
				if cid == startClass {
					targetYears = []string{"2023-24", "2024-25", "2025-26", "2026-27"}
	...
	for records := range resultChan {
		...
		// Realtime update dynamically based on strict task completion indices
